      ...
      <div class="card-body" id="body">
        ...
        <div class="col-12" style="overflow-x:auto;">
          <table id="myTable" class="display">
            ...
            <tbody> @foreach ($user as $data => $users) <tr id="index_{{ $users->id }}">
                <td>{{$data + 1}}</td>
                <td>
                  <img src="{{$users->foto_profile}}" class="img-thumbnail rounded" alt="">
                </td>
                <td>{{$users->name}}</td>
                <td>{{$users->email}}</td>
                <td class="text-center">
                  <a href="javascript:void(0)" id="btn-edit-post" data-id="{{ $users->id }}" class="btn btn-primary ">Edit</a> @method('DELETE') <a href="{{ route('manajemen_user.destroy', $users->id) }}" class="btn btn-danger" data-confirm-delete="true">Delete</a>
                </td>
              </tr> @endforeach </tbody>
          </table>
        </div>
