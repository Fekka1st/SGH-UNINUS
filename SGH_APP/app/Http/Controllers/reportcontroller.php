    ...
    public function generateReport(Request $request)
    {
        ...
        if ($deviceType == "greenhouse") {
            $data = DB::table($tableName)
                ->select(
                    DB::raw(
                        "$selectColumns, DATE_FORMAT(greenhouse_sensor_datas.created_at, '%Y-%m-%d %H:00:00') as hour_block"
                    )
                )
                ->leftJoin("extend_devices", function ($join) {
                    $join
                        ->on(
                            "greenhouse_sensor_datas.created_at",
                            "=",
                            "extend_devices.created_at"
                        )
                        ->where(
                            "extend_devices.name_device",
                            "=",
                            "water_level"
                        );
                })
                ->whereRaw(
                    "greenhouse_sensor_datas.created_at >= NOW() - INTERVAL $interval"
                )
                ->groupBy("hour_block")
            ...
            $data = DB::table($tableName)
                ->select(
                    DB::raw(
                        "$selectColumns, DATE_FORMAT(created_at, '%Y-%m-%d %H:00:00') as hour_block"
                    )
                )
